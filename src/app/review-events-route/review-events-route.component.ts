...
@Component({
  selector: 'app-review-events-route',
  templateUrl: './review-events-route.component.html',
  styleUrls: ['./review-events-route.component.css']
})
export class ReviewEventsRouteComponent implements OnInit {
  ...
  fetchEventAmount() {
    this.position = 0;
    this.isLoading = true;
    this.reviewEventService.getEventAmount(this.searchString, this.searchDate, this.descending).subscribe(amount => {
  ...
  fetchEventList() {
    ...
    this.isLoading = true;
    this.reviewEventService.getEvents(this.position, this.searchString, this.searchDate, this.descending).subscribe(events => {
      this.errorMessage = null;
